...
def downloader(link, output_file):
    command = " ".join(["ffmpeg", "-i", link, "-bsf:a aac_adtstoasc -vcodec copy -c copy", f"{os.path.dirname(os.path.abspath(__file__))}\\{output_file}"])
    print(command)
    ...
    if input("Start downloading?.. (y/n) ").strip().lower() == "y":
        ...
        print(f"Script running time: {timer//3600}h " * (timer>=3600) + f"{(timer//60)%60}m " * (timer>=60 or timer>=3600) + f"{timer%60}s")
